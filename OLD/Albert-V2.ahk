...
; F1 запускает действие текущего режима. В ZIK: завершить текущий звонок → следующий номер.
...
; F2 выполняет действие текущего режима. В ZIK: KOCHADAN → завершить звонок → следующий номер.
; Во всех остальных режимах существующая логика сохраняется без изменений.
$F2::
{
...
; Launch_Media: в режиме ZIK выполняет KOCHADAN → завершить звонок → следующий номер.
...
$Launch_Media::
{
    ...
    ; ZIK: Launch_Media = KOCHADAN → завершить звонок → следующий номер.
...
$Browser_Home::
{
    ...
    ; ZIK: Browser_Home = завершить текущий звонок → следующий номер.
    if (currentMode == 6)
    {
        ZikF1Next()
        return
